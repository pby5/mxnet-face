...
def group1(data, num_r, num, kernel, stride, pad, layer):
    if num_r > 0:
        conv_r  = mx.symbol.Convolution(data=data, num_filter=num_r, kernel=(1,1), name=('conv%s_r' % layer))
        slice_r = mx.symbol.SliceChannel(data=conv_r, num_outputs=2, name=('slice%s_r' % layer))
        mfm_r   = mx.symbol.maximum(slice_r[0], slice_r[1])
        ...
        conv = mx.symbol.Convolution(data=data, kernel=kernel, stride=stride, pad=pad, num_filter=num, name=('conv%s' % layer))
    pool  = mx.symbol.Pooling(data=conv, pool_type="max", kernel=(2, 2), stride=(2,2), name=('pool%s' % layer))
    slice = mx.symbol.SliceChannel(data=pool, num_outputs=2, name=('slice%s' % layer))
    mfm   = mx.symbol.maximum(slice[0], slice[1])
    return mfm
...
def lightened_cnn_a_feature1():
    data = mx.symbol.Variable(name="data")
    pool1 = group1(data, 0, 96, (9,9), (1,1), (0,0), str(1))
    pool2 = group1(pool1, 0, 192, (5,5), (1,1), (0,0), str(2))
    pool3 = group1(pool2, 0, 256, (5,5), (1,1), (0,0), str(3))
    pool4 = group1(pool3, 0, 384, (4,4), (1,1), (0,0), str(4))
...
def lightened_cnn_a1(num_classes=10575):
     drop1   = lightened_cnn_a_feature1()
     fc2     = mx.symbol.FullyConnected(data=drop1, num_hidden=num_classes, name="fc2")
     softmax = mx.symbol.SoftmaxOutput(data=fc2, name='softmax')
     return softmax
...
def lightened_cnn_b_feature1():
     data = mx.symbol.Variable(name="data")
     pool1 = group1(data, 0, 96, (5,5), (1,1), (2,2), str(1))
     pool2 = group1(pool1, 96, 192, (3,3), (1,1), (1,1), str(2))
     pool3 = group1(pool2, 192, 384, (3,3), (1,1), (1,1), str(3))
     pool4 = group1(pool3, 384, 256, (3,3), (1,1), (1,1), str(4))
     pool5 = group1(pool4, 256, 256, (3,3), (1,1), (1,1), str(5))
     flatten   = mx.symbol.Flatten(data=pool5)
     fc1       = mx.symbol.FullyConnected(data=flatten, num_hidden=512, name="fc1")
     slice_fc1 = mx.symbol.SliceChannel(data=fc1, num_outputs=2, name="slice_fc1")
     mfm_fc1   = mx.symbol.maximum(slice_fc1[0], slice_fc1[1])
...
def lightened_cnn_b1(num_classes=10575):
     drop1   = lightened_cnn_b_feature1()
     fc2     = mx.symbol.FullyConnected(data=drop1, num_hidden=num_classes, name="fc2")
     softmax = mx.symbol.SoftmaxOutput(data=fc2, name='softmax')
     return softmax
...
def main():
    ...
    #lightened_cnn = lightened_cnn_a1()
    #lightened_cnn = lightened_cnn_b()
    lightened_cnn = lightened_cnn_b1()
    ...
    model = mx.model.FeedForward(
        ...
        lr_scheduler       = mx.lr_scheduler.FactorScheduler(step=5*max(int(epoch_size * 1), 1), factor=0.8),
